...
| **IAM Permissions** | Shared `apiLambdaRole` across all Lambda functions with wildcard resources and cdk-nag suppressions. | Create per-function least-privilege roles. Scope to specific model ARNs, DynamoDB table ARNs, and S3 bucket ARNs. If one handler is compromised, blast radius is limited to its own resources. |
...
| **Chat Guardrails** | `chat_handler.py` passes user input to Bedrock without applying the Bedrock Guardrail (env var exists but unused) and has no prompt injection scanning. | Apply Bedrock Guardrails on all chat invocations. Add input sanitization with the same injection detection used in claims processing. Prevents knowledge base exfiltration and prompt manipulation. |
| **OpenSearch Network Policy** | OpenSearch Serverless collection uses `AllowFromPublic: true` for network access. | Restrict to VPC endpoint access only. RAG embeddings (fraud patterns, policy rules, regulatory content) should not be reachable outside the application layer. |
